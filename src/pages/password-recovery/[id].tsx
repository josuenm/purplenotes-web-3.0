...
export default function PasswordRecovery() {
  const { errorNotification, successNotification } = useGlobalTools();
  const router = useRouter();
  const { id } = router.query;
  const [isLoading, setIsLoading] = useState(false);

  if (!id || typeof id !== "string") return router.push("/");

  const checkPasswordRecovery = useCallback(async () => {
    setIsLoading(true);
    const res = await userApi.getPasswordRecovery(id);

    switch (res.status) {
      case 200:
        if (res.data.isUsed) {
          router.push("/sign-in");
          errorNotification("Password recovery already used");
          break;
        }
        if (Date.now() > new Date(res.data.expiryDate).getTime()) {
          router.push("/sign-in");
          errorNotification("Password recovery expired");
          break;
        }
        break;
  ...
  const submit = async (data: PasswordInfoData) => {
    setIsLoading(true);
    const res = await userApi.confirmPasswordRecovery(id, {
    ...
    switch (res.status) {
      ...
      case 409:
        errorNotification("You are already using this password");
        break;
  ...
  return (
    <>
      ...
      <main className="safe-area !max-w-[330px] min-h-screen flex flex-col justify-center">
        <h2 className="text-xl text-center self-center font-medium rounded-md w-fit">
